<template>
    <div>
            ...
            <div class="card">
                <div class="card-item">
                    ...
                    <div class="card-item-content">
                        <input type="text" class="input" placeholder="Nhập tên sản phẩm..." title="Tìm kiếm sản phẩm" v-model="search">
                        <button class="button" @click="findProduct">Tìm kiếm</button>
        ...
        <ProductForm title="sản phẩm" :status="status" :item="item" v-if="showForm" @close="closeForm" @success="success"></ProductForm>
...
export default {
    ...
    data() {
        return {
            tableHeader:[{id:"id",name:"Mã"},{id:"name",name:"Tên sản phẩm"},{id:"price",name:"Giá sản phẩm"},{id:"amount",name:"Số lượng còn"}],
    ...
    methods:{
        ...
        async getProduct(){
            ...
            this.hasLoader = true
            let data = await this.$axios.get(`${api.ProductApi}/pagination`,{params:param}).then(res=>res.data).finally(()=>this.hasLoader=false)
                console.log(data);
